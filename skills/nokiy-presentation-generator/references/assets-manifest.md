...
Set `TWS_ASSET_LIBRARY_PATH` or pass `--asset-library` for a local library.
For remote use, set `TWS_ASSET_LIBRARY_BASE_URL` and the pinned catalog digest;
do not embed an operator-specific absolute path in the Skill.
...
verify it against this root before copying or embedding any selected file.
