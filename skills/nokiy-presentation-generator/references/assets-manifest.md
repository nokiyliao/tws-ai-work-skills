...
Company learners do not configure the shared asset-library path. The
orchestrator materializes a verified job-local snapshot from the remote service.
Project and output roots remain job-specific and may be selected by the learner.
...
- Verified reusable TWS asset library
...
The administrator injects `TWS_ASSET_LIBRARY_BASE_URL` and the pinned catalog
digest. The learner-facing Skill never requests or stores the server path.

Do not copy catalog entries into this skill or hard-code individual image paths.
Consume a per-deck selection manifest generated by the library selector, then
verify it against the materialized snapshot before embedding any selected file.
