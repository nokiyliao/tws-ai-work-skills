...
處理 TWS 平台與客戶簡報時，除了本 Skill 的通用規則，還必須遵守以下邊界：

- 保留來源中的公司名、產品名、型號、數字、單位、日期、技術名詞、引文、引用與使用者鎖定文字。
- 不得為了讓句子更順而提高原始主張強度；資料不足時保留 `待確認`、`初步盤點`、`依現場條件評估` 等限定語。
- 標題、內文、講者備註、圖說、流程節點、按鈕與 CTA 都不得使用空泛的對比或口號句型。
- `先……再……`、標題中的 `先＋動作`、`不是……而是……`、`不只……`、`不僅……而且……`、`真正的……` 預設視為不合格，除非它是引用、必要術語或真實依賴順序。
...
| --- | --- |
| `物料流先走一遍` | `盤點當前物料流` |
| `這不是公司介紹，而是改善藍圖` | `本提案整理現況、改善範圍與導入條件` |
| `不只提升效率，更創造真正的價值` | `縮短搬運時間，降低重複作業與人車交會風險` |

若鎖定文字與規則衝突，標記衝突並保留原文，不得靜默改寫。TWS 簡報中仍殘留上述公式句型，Humanizer gate 應判定失敗。
...
### 9A. 內部課程、商務簡報與網站文案的禁用句型

**適用範圍：** 課程標題、段落標題、簡報標題、簡報內文、講者備註、網站文案、圖片標註與流程說明。

**硬性規則：** 成品不得使用下列公式化句型。即使語法正確，也要改成具體的動作、對象或成果。
...
**判斷原則：** 不只檢查完整模板，也要檢查變形句。單獨使用「先」來製造口號、流程感或戲劇性鋪陳，同樣需要改寫。若「先」是不可省略的實際時序條件，應直接寫明時間、前置條件或作業順序，避免口號式表達。

**改寫方法：**

- 直接寫工作動作：盤點、核對、分類、計算、覆核、匯出。
- 寫清楚對象：物料流、牌價表、客戶信件、設備規格、案件資料。
- 寫出成果或驗收條件：差異表、待辦清單、可下載簡報、來源列號。
- 必須說明順序時，使用編號、時間或明確前置條件，不用「先／再」當修辭骨架。

**改寫前 → 改寫後：**

- 「物料流先走一遍」 → 「盤點當前物料流」
- 「先分類，再查核，後回覆」 → 「完成案件分類、資料查核與回覆草稿」
- 「教的是建立順序，不是發一套標準答案」 → 「課程協助學員建立個人判斷順序與仲裁規則」
- 「AI 不是聊天工具，而是工作夥伴」 → 「AI 可協助整理資料、產出初稿與執行重複工作」
- 「不只整理信件，還能產生待辦」 → 「整理信件並產生待辦清單」
- 「真正的 AI 導入從工作流開始」 → 「AI 導入範圍涵蓋資料、操作步驟、人工覆核與交付成果」
...
- ✓ **使用了「此外」「然而」等連接詞？** 考慮刪除
- ✓ **三段式列舉？** 改為兩項或四項
- ✓ **出現「先／再」「不是／而是」「不只」「真正的」等模板？** 改成具體動作、對象或成果
